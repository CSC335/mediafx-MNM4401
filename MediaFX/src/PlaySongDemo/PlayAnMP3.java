 ...
 * 
 * Programmer Rick Mercer & Mahesh Magendaran
 */
import java.io.File;
import java.net.URI;
...
public class PlayAnMP3 extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		// Mahesh changed the song to swing cheese before push
		String path = "songfiles/SwingCheese.mp3";
		// lines 33 and 34 were swapped, so savage
		playASong(path);
	...
	private void playASong(String path) {
		...
		// changed the var name file t0 fileInput
		File fileInput = new File(path);
		URI uri = fileInput.toURI();
		...
		// added a different message
		System.out.println("Well that was unexpected, but the program might need to be killed");
